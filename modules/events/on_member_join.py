...
class OnMemberJoin(commands.Cog):
    ...
    async def on_member_join(self, member):
        Log.info("event", "on_member_join", f"User '{member.name}' ({member.id}) has joined the server")
        ...
        Log.info("event", "on_member_join", f"The greating embed for membed '{member.name}' was sended to the channel '{channel.id}'")
